package ru.t1.java.demo.config.kafka.client;
...
public class KafkaClientConfig {
    ...
    @Value("${spring.t1.kafka.consumer.client-group-id}")
    ...
    @Value("${spring.t1.kafka.topic.client_registration}")
    ...
    public ConsumerFactory<String, ClientDto> clientConsumerListenerFactory() {
        ...
        props.put(JsonDeserializer.VALUE_DEFAULT_TYPE, "ru.t1.java.demo.model.dto.client.ClientDto");
    ...
    @Bean
    ConcurrentKafkaListenerContainerFactory<String, ClientDto> clientKafkaListenerContainerFactory(@Qualifier("clientConsumerListenerFactory") ConsumerFactory<String, ClientDto> consumerFactory) {
    ...
    @Bean
    @Primary
    public KafkaTemplate<String, Long> kafkaClientTemplate(@Qualifier("producerClientFactory") ProducerFactory<String, Long> producerPatFactory) {
    ...
    @ConditionalOnProperty(value = "t1.kafka.producer.enable",
            havingValue = "true",
            matchIfMissing = true)
    public KafkaClientProducer<ClientDto> producerClient(@Qualifier("kafkaClientTemplate") KafkaTemplate<String, Long> template) {
        template.setDefaultTopic(clientTopic);
        return new KafkaClientProducer<>(template);
    }

    @Bean("producerClientFactory")
    public ProducerFactory<String, Long> producerClientFactory() {
        Map<String, Object> props = new HashMap<>();
