...
public class DemoKafkaConfig<T> {
    ...
    @ConditionalOnProperty(value = "t1.kafka.producer.enable",
            havingValue = "true",
            matchIfMissing = true)
    public KafkaClientProducer<LogDataSourceErrorAspect> producerClient(@Qualifier("client") KafkaTemplate<String, ClientDto> template) {
        template.setDefaultTopic(clientTopic);
        return new KafkaClientProducer<LogDataSourceErrorAspect>(template);
    }
