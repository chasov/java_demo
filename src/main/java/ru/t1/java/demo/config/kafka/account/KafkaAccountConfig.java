package ru.t1.java.demo.config.kafka.account;
...
public class KafkaAccountConfig {
    ...
    @Value("${spring.t1.kafka.consumer.account-group-id}")
    ...
    @Value("${spring.t1.kafka.topic.client_accounts}")
    ...
    public ConsumerFactory<String, AccountDto> accountConsumerListenerFactory() {
        ...
        props.put(JsonDeserializer.VALUE_DEFAULT_TYPE, "ru.t1.java.demo.model.dto.account.AccountDto");
    ...
    @ConditionalOnProperty(value = "t1.kafka.producer.enable",
            havingValue = "true",
            matchIfMissing = true)
    public KafkaAccountProducer<AccountDto> producerAccounts(@Qualifier("kafkaAccountTemplate") KafkaTemplate<String, AccountDto> template) {
        template.setDefaultTopic(accountsTopic);
        return new KafkaAccountProducer<>(template);
    }

    @Bean("producerAccountsFactory")
    public ProducerFactory<String, AccountDto> producerAccountsFactory() {
        Map<String, Object> props = new HashMap<>();
